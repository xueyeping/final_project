...
# from pcgrad import PCGrad   # gradient surgery
...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.linear2 = nn.Linear(2*config.hidden_size, 1)
        self.linear3 = nn.Linear(2*config.hidden_size, 1)
    ...
    def predict_paraphrase(self,
        ...
        hidden_2 = self.dropout2(hidden_2)
    ...
    def predict_similarity(self,
        ...
        '''
        hidden_1 = self.forward(input_ids_1, attention_mask_1)
        hidden_1 = self.dropout3(hidden_1)
        hidden_2 = self.forward(input_ids_2, attention_mask_2)
        hidden_2 = self.dropout3(hidden_2)
        hidden = torch.cat((hidden_1, hidden_2), dim=1)
        logits = self.linear3(hidden)
        return logits
...
def train_multitask(args):
    ...
    for epoch in range(args.epochs):
        ...
        sst_acc,_, _, para_acc, _, _, sts_corr, _, _ = model_eval_multitask(sst_dev_dataloader, para_dev_dataloader, sts_dev_dataloader, model, device)
        dev_acc = (sst_acc + para_acc + sts_corr)/3
        
        if dev_acc > best_dev_acc:
            best_dev_acc = dev_acc
            print(f"!!!!!!Best average dev accuracy is {dev_acc}")
