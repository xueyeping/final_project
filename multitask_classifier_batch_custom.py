...
def train_multitask(args):
    ...
    for epoch in range(args.epochs):
        ...
        for sst_batch, para_batch, sts_batch in tqdm(zip(sst_train_dataloader, para_train_dataloader, sts_train_dataloader), desc=f'train-{epoch}', disable=TQDM_DISABLE):
            ...
            sst_loss = F.cross_entropy(logits, b_labels.view(-1), reduction='sum') / args.batch_size_sst
            ...
            para_loss = F.binary_cross_entropy_with_logits(logits.view(-1), b_labels.float(), reduction='sum') / args.batch_size_para
            ...
            sts_loss =  F.mse_loss(logits.view(-1), b_labels.float(), reduction='sum') / args.batch_size_sts
...
def get_args():
    ...
    parser.add_argument("--batch_size_sst", type=int, default=8)
